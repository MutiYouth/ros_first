...
class CodeGlanceConfigurable : BoundSearchableConfigurable("Code Glance Pro", "com.weng.ros_first.code_glance_pro") {
                ...
                twoColumnsRow(
                    {
                        comboBox(DefaultComboBoxModel(MouseJumpEnum.values().map { it.getMessage() }.toTypedArray()))
                            ...
                            .bindItem({ config.jumpOnMouseDown.getMessage() },
                                { config.jumpOnMouseDown = MouseJumpEnum.findMouseJumpEnum(it) })
                ...
                twoColumnsRow(
                    {
                        textField().label(message("settings.viewport.color"))
                            ...
                            .validationOnInput {
                                if (it.text.length != 6 || !"[a-fA-F\\d]{6}".toRegex()
                                        .matches(it.text)
                                ) error(message("settings.viewport.error"))
                                else null
                            }
                    },
                    {
