        ...
        loop@ while (!hlIter.atEnd() && !editor.isDisposed) {
            ...
            if (region != null) {
                ...
                if (region !is CustomFoldRegionImpl) {
                    ...
                    do hlIter.advance() while (!hlIter.atEnd() && hlIter.start < endOffset)
                    myRangeList.value.add(
                        Pair(
                            editor.offsetToVisualLine(endOffset),
                    ...
                    text.subSequence(
                        start, if (lineCount <= line) endOffset else {
                ...
                for (offset in start until end) {
                    ...
                    moveCharIndex(charCode) {
                        if (hasBlockInlay) {
                            ...
                            val sumBlock = editor.inlayModel.getBlockElementsInRange(
                                startOffset,
                                DocumentUtil.getLineEndOffset(startOffset, editor.document)
                            )
                            ...
                            if (sumBlock > 0) {
                                myRangeList.value.add(
                                    Pair(
                                        editor.offsetToVisualLine(startOffset) - 1,
                                        Range(y, y + sumBlock)
                                    )
                                )
                                y += sumBlock
