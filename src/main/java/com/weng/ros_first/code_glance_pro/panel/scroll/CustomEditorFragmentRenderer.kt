...
import com.weng.ros_first.code_glance_pro.panel.scroll.ScrollBar.Companion.PREVIEW_LINES
...
class CustomEditorFragmentRenderer(private val myEditor: EditorImpl) {
    ...
    private fun showEditorHint(point: Point, hintInfo: HintHint) {
        ...
        HintManagerImpl.getInstanceImpl()
            .showEditorHint(myEditorPreviewHint!!, myEditor, point, flags, 0, false, hintInfo)
    }
    ...
    private inner class EditorFragmentPreviewPanel(
        private val myContentInsets: Int,
        private val myHighlighters: MutableList<RangeHighlighterEx>
    ) : JPanel() {
        ...
        override fun getPreferredSize(): Dimension {
            var width =
                ((myEditor.gutterComponentEx as EditorGutterComponentEx).width + myEditor.scrollingModel.visibleArea.width
        ...
        override fun paintComponent(g: Graphics) {
            ...
            if (myCacheLevel2 == null) {
                ...
                myCacheLevel2 =
                    ImageUtil.createImage(g, size.width, myCacheToY - myCacheFromY, BufferedImage.TYPE_INT_RGB)
                ...
                val contentAT =
                    AffineTransform.getTranslateInstance((gutterWidth - shift).toDouble(), lineShift.toDouble())
            ...
            if (myCacheLevel1 == null) {
                myCacheLevel1 = ImageUtil.createImage(
                    g,
                    size.width,
                    lineHeight * (2 * PREVIEW_LINES + 1),
            ...
            if (isDirty) {
                ...
                val translateInstance = AffineTransform.getTranslateInstance(
                    gutterWidth.toDouble(),
                    (myCacheFromY - topDisplayedY).toDouble()
                )
                ...
                for (ex in myHighlighters) {
                    ...
                    g2d.drawString(
                        s,
                        placeToShow.x + R,
                        placeToShow.y + h - g2d.getFontMetrics(g2d.font).descent / 2 - 2
                    )
                }
                isDirty = false
            }
            ...
            try {
                ...
                if (StartupUiUtil.isUnderDarcula()) {
                    ...
                    val cy = 0.0
                    val topShape1 =
                        Ellipse2D.Double(cx - rx - pseudoMajorAxis / 2, cy - ry, 2 * rx, (2 * ry).toDouble())
                    val topShape2 =
                        Ellipse2D.Double(cx - rx + pseudoMajorAxis / 2, cy - ry, 2 * rx, (2 * ry).toDouble())
