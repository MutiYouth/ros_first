package com.weng.ros_first.code_glance_pro.panel.vcs

import com.intellij.openapi.editor.ScrollType
import com.intellij.openapi.editor.VisualPosition
import com.intellij.openapi.util.Disposer
import com.weng.ros_first.code_glance_pro.listener.MyVcsListener
...
class MyVcsPanel(val glancePanel: GlancePanel) : JPanel() {
    ...
    private inner class MouseHandler : MouseAdapter() {
        ...
        override fun mouseMoved(e: MouseEvent) {
            ...
            val process = editor.filteredDocumentMarkupModel.processRangeHighlightersOverlappingWith(
                rangeOffset.from,
                rangeOffset.to
            ) {
                if (it.isThinErrorStripeMark) it.getErrorStripeMarkColor(editor.colorsScheme)?.apply {
                    val visualLine = fitLineToEditor(
                        editor,
                        glancePanel.getMyRenderVisualLine(e.y + glancePanel.scrollState.visibleStart)
                    )
